...
function mousePressEvent(event) {
    if (alt && !isActive) {
        ...
        var rayDistance = PickRay.distance;
        var rayIntersect = PickRay.intersection;
        trackAvatar = null;
        switch (PickRay.type) {
            case Picks.INTERSECTED_NONE:
                //console.log("Ray Pick [None]: " + PickRay.distance + " : " + modelIntersection.distance);
                break;
            case Picks.INTERSECTED_ENTITY:

                break;
            case Picks.INTERSECTED_OVERLAY:

                break;
            case Picks.INTERSECTED_AVATAR:
                //console.log("AVATAR HIT");
                var avatar = AvatarList.getAvatar(PickRay.objectID);
                if (avatar.sessionUUID === PickRay.objectID) {
                    trackAvatar = PickRay.objectID;
                    rayIntersect = avatar.position;
                    rayIntersect.y += 0.5;
                    rayDistance = Vec3.distance(position, rayIntersect);
                    updateCenterTimer = Script.setInterval(updateCenter, 10);
                    //console.log("AVATAR CAM");
                }
                break;
            case Picks.INTERSECTED_HUD:
        ...
        if (PickRay.intersects) {
            ...
            center = Vec3.sum(Camera.position, Vec3.multiply(ORBIT_DISTANCE, PickRay.searchRay.direction));
        } {
            // ...CAS
...
var updateCenterTimer = null;

function updateCenter() {
    if (!isActive && !notResetYet) {
        Script.clearInterval(updateCenterTimer);
        console.log("STOP UPDATE BECASUE NOT ACTIVE");
        return;
    }
    if (trackAvatar !== null) {
        console.log("TICK AVATAR");
        var avatar = AvatarList.getAvatar(trackAvatar);
        if (avatar.sessionUUID === trackAvatar) {
    ...
    mouseMoveEvent({x:mouseLastX,y:mouseLastY});
}
