function SVGTransform() {
  ...
  that.transforms = {};
  that.string = "";
  that.generic = function(trans, args) {
    that.transforms[trans] = args;
    that.updateString();
  };

  that.rotate = function(deg, x, y) {
    that.transforms.rotate = [deg, x, y];
    that.updateString();
  };

  that.scale = function(x, y) {
    that.transforms.scale = [x, y];
    that.updateString();
  };
...
function SVGStyle() {
  ...
  that.string = "";
  that.styles = {};
  that.generic = function(style, args) {
    that.styles[style] = args;
    that.updateString();
  };

  that.removeStyle = function(style) {
    delete(that.styles[style]);
    that.updateString();
  };
